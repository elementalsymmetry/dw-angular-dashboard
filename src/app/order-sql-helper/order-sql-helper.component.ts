...
//    let temp1 = "db2 \"select orders_id,ormorder,status,timeplaced,lastupdate from orders where orders_id in (" + temp.toString() + ")\"\n";
    let temp1 = `db2 "select DISTINCT orders.orders_id,orders.ormorder,orders.status,orderblk.resolved,orderitems.shipmode_id, shipmode.carrier,orders.timeplaced,orders.lastupdate from orders left outer join orderblk on (orders.orders_id = orderblk.orders_id) left outer join orderitems on (orderitems.orders_id = orders.orders_id) left outer join shipmode on (orderitems.shipmode_id = shipmode.shipmode_id) where orders.orders_id in (${temp}) order by orders_id"\n`;
